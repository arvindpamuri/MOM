		...
		<div class="row ">
			...
			<div class="col-lg-5 description-half" id = "Sale">
				<div class="container">
						...
						echo'
							<article class="removed" style = "display: none; " id = "'.$row1['p_id'].'">
								<header>
									<a class="remove">
								...
								<div class="prodid">'.$row1['p_id'].'</div>

								<footer class="content count">
					...
					<footer id="site-footer">
						<div class="container clearfix">
							...
							<div class="right">
								...
								<form name="send_form" id="formoid" action="send.php" method="post" >
								<input type = "hidden" name="p_id"  id="p_id" value="">
								<input type = "hidden" name="p_quant" id="p_quant" value="">
								<input type = "hidden" name="p_total" id="total" value="">
								    <button id ="send" name="submit" type='submit' class="btn btn-primary checkout" ;>Checkout</button>
...
<script type="text/javascript">
document.getElementById("p_id").value = JSON.stringify(product_id);
document.getElementById("p_quant").value = JSON.stringify(product_quant);
document.getElementById("total").value = JSON.stringify(totalVal);
 $("#formoid").submit(function(event) {
      ...
      var posting = $.post( url, { product_id: $('#p_id').val(), product_quant: $('#p_quant').val(),total:$('#total') } );
...
function updateform() {
    ...
    var z = "#" + x;
    $(z).removeClass('removed').addClass('product');
	}
	var product_id = new Array();
    var product_quant = new Array();
   
    var totalVal = 0;
    $(".checkout").click(function(){
        $(".product").each(function(index){
	    var temp = $(this).children(".prodid").html();
	    var tempquant = $(this).children(".count").children(".qt").html();
	    if(tempquant > 0){
	    
	        product_id.push(temp);
	        product_quant.push(tempquant);
	    }
	    ...
	    window.setTimeout(
	      function(){
	        el.parent().parent().slideUp('fast', function() { 
	          el.parent().parent().children('.content').children('.full-price').html('0');
	          el.parent().parent().children('.content').children(".qt").html('0');
	          if($(".product").length == 0) {
	            if(check) {
